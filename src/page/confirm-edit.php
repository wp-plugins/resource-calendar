...
class Confirm_Edit extends ResourceCalendar_Page {
	...
	public function check_request() {
		if (wp_verify_nonce($_POST['nonce'],session_id()) === false) {
			throw new Exception(ResourceCalendar_Component	::getMsg('E012',basename(__FILE__).':'.__LINE__),1 );
		}
		...
		if ($this->datas['check_day'] < $now )  {
			throw new Exception(ResourceCalendar_Component	::getMsg('E011',$this->datas['target_day'].' '.$this->datas['time_from']));
		}
